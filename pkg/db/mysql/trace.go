...
func (c *callbacks) after(scope *gorm.DB, operation string) {
	...
	if operation == "" {
		operation = strings.ToUpper(strings.Split(st.SQL.String(), " ")[0])
	}
	if scope.Error != nil {
		ext.Error.Set(sp, true)
		sp.SetTag("db.error", scope.Error.Error())

	}
	sql := st.SQL.String()
	if strings.Contains(sql, "?") {
		// replace ? to var
		sqlLayout := strings.Replace(sql, "?", "'%v'", -1)
		sql = fmt.Sprintf(sqlLayout, st.Vars...)
	}
	ext.DBStatement.Set(sp, sql)
	sp.SetTag("db.vars", st.Vars)
	sp.SetTag("db.table", st.Table)
	sp.SetTag("db.method", operation)
	sp.SetTag("db.count", st.RowsAffected)
	sp.Finish()
}
